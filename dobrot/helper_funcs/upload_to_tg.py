...
from dobrot.helper_funcs.upin import url_up
from dobrot.helper_funcs.Convert import convert_video
from dobrot import (
    TG_MAX_FILE_SIZE,
    EDIT_SLEEP_TIME_OUT,
    DOWNLOAD_LOCATION,
...
async def upload_single_file(message, local_file_name, caption_str, from_user):
    await asyncio.sleep(10)#EDIT_SLEEP_TIME_OUT)
    sent_message = None
    ...
    try:
        ...
        if local_file_name.upper().endswith(("AVIkotanilohdhatydiondgduf")):
            ...
            if caption_str in local_file_name:
                ...
                caption_str2 = caption_str1
                if "%20" in caption_str2:
                   caption_str2 = caption_str2.replace("%20"," ").replace("%27","'")
                caption_str2 = await remove_w(caption_str2)
                yash = local_file_name.replace(caption_str, caption_str2)
                #yash = await remove_w(yash)
                local_file_name = os.rename(local_file_name, yash)
